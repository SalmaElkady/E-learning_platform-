...
function login() {
    ...
    if(!name || !email || !role) { alert("Please enter name, email and role"); return; }
    if(!email.includes("@")) { alert("Invalid email"); return; }

    if(role === "student") {
        const allowed = students.find(s => s.name === name && s.email === email);
        if(!allowed){ alert("Access denied: student not registered"); return; }
        ...
        if(!allowed){ alert("Access denied: instructor not registered"); return; }
        localStorage.setItem("role", role);
        localStorage.setItem("name", name);
        window.location.replace("../pages/instructor/index.html");
    } else if(role === "admin") {
        const allowed = Admins.find(a => a.name === name && a.email === email);
        if(!allowed){ alert("Access denied: admin not registered"); return; }
        localStorage.setItem("role", role);
        localStorage.setItem("name", name);
